"""
법령해석례 전수 수집 → Vertex AI Search 임포트.

법제처 DRF API로 법령해석례 전체(~8,600건)를 본문 포함 수집하여
Vertex AI Search 데이터스토어에 추가.

사용법:
    # 전체 파이프라인
    python scripts/fetch_expc_full.py

    # 단계별
    python scripts/fetch_expc_full.py --step list
    python scripts/fetch_expc_full.py --step detail
    python scripts/fetch_expc_full.py --step transform
    python scripts/fetch_expc_full.py --step import
"""
...
# Output
OUTPUT_DIR = os.path.join(BASE_DIR, "vertex_expansion", "expc_full")
LIST_DIR = os.path.join(OUTPUT_DIR, "list")
DETAIL_DIR = os.path.join(OUTPUT_DIR, "detail")
JSONL_PATH = os.path.join(OUTPUT_DIR, "expc_full.jsonl")
PROGRESS_FILE = os.path.join(OUTPUT_DIR, "progress.json")

# GCS / Vertex
GCS_BUCKET = "gs://lawmadi-vertex-data"
GCS_PATH = f"{GCS_BUCKET}/expc_full.jsonl"
PROJECT_ID = "lawmadi-db"
LOCATION = "global"
DATA_STORE_ID = "lawmadi-legal-cache"

# 수집 설정
TARGET_COUNT = 9000  # 전수 (현재 8,640건 + 여유)
PAGE_SIZE = 100
DETAIL_DELAY = 0.4
LIST_DELAY = 0.3
...
def step_list():
    ...
    print(f"\n[1/4] 법령해석례 목록 수집 (목표: 전수)")

    while total < TARGET_COUNT:
        url = (
            f"{DRF_BASE}/lawSearch.do?"
            f"OC={OC}&target=expc&type=JSON"
        ...
        items = data.get("Expc", data.get("ExpcSearch", data)).get("expc", [])
        if not items:
...
def step_detail():
    ...
    for i, serial in enumerate(serials):
        ...
        url = (
            ...
            f"OC={OC}&target=expc&ID={serial}&type=JSON"
        )
        data = _api_get(url, retries=3, delay=2.0)

        if data and "ExpcService" in data:
            with open(out_path, "w", encoding="utf-8") as f:
                json.dump(data["ExpcService"], f, ensure_ascii=False, indent=2)
            success += 1
        else:
            fail += 1
...
def step_transform():
    ...
    with open(JSONL_PATH, "w", encoding="utf-8") as out:
        for fname in sorted(os.listdir(DETAIL_DIR)):
            ...
            case_name = svc.get("안건명", "")
            case_no = svc.get("안건번호", "")
            interp_date = svc.get("해석일자", "")
            interp_org = svc.get("해석기관명", "")
            query_org = svc.get("질의기관명", "")
            query_summary = svc.get("질의요지", "")
            answer = svc.get("회답", "")
            reason = svc.get("이유", "")

            if not case_name and not query_summary:
                continue
            ...
            if query_summary:
                content_parts.append(f"질의요지: {query_summary}")
            if answer:
                content_parts.append(f"회답: {answer}")
            if reason:
            ...
            doc = {
                "id": doc_id,
                "structData": {
                    "ssot_type": "legal_interpretation",
                    "law_name": case_name,
                    "label": "법령해석례",
                    "target": "expc",
                    "endpoint": "lawService.do",
                    "case_no": case_no,
                    "date": str(interp_date),
                    "serial": serial,
                    "interp_org": interp_org,
                    "query_org": query_org,
...
def main():
    parser = argparse.ArgumentParser(
        description="법령해석례 전수 수집 → Vertex AI Search"
    )
    parser.add_argument(
        "--step",
    ...
    print("=" * 60)
    print("법령해석례 전수 수집 → Vertex AI Search")
    print("=" * 60)
