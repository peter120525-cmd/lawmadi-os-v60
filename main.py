...
# =============================================================
# 🌊 /ask/stream - 스트리밍 응답 (체감 속도 300% 개선)
# =============================================================
...
async def ask_stream(req: Request):
    ...
    async def generate():
        try:
            ...
            # Low Signal 차단
            if _is_low_signal(query):
                yield f"data: {json.dumps({'type': 'chunk', 'text': '[마디 통합 리더 답변]\\n\\n'})}\n\n"
                yield f"data: {json.dumps({'type': 'chunk', 'text': '1. 요약: 서버 정상 동작 중 ✅\\n\\n'})}\n\n"
                yield f"data: {json.dumps({'type': 'done', 'leader': '마디', 'latency_ms': 100})}\n\n"
                return

            # Gemini 키 확인
            if not os.getenv("GEMINI_KEY"):
                yield f"data: {json.dumps({'type': 'error', 'message': 'GEMINI_KEY 미설정'})}\n\n"
                return

            # 메타데이터 전송 (시작)
            yield f"data: {json.dumps({'type': 'start', 'trace_id': trace})}\n\n"
            ...
            yield f"data: {json.dumps({'type': 'done', 'leader': leader_name, 'latency_ms': latency_ms, 'chunks': chunk_count}, ensure_ascii=False)}\n\n"
            ...
            _audit("ask_stream", {
                ...
                "status": "SUCCESS",
                "latency_ms": latency_ms,
                "response_sha256": _sha256(full_text),
            })
            ...
            try:
                ...
                logger.warning(f"⚠️ [ChatHistory] 저장 실패: {log_error}")

        except Exception as e:
            logger.error(f"💥 [Stream] 에러: {e}")
