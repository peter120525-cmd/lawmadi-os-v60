...
def set_dependencies(
    ...
    *,
    ...
    global _RUNTIME, _LEADER_REGISTRY, _LEADER_PROFILES
...
def _build_system_instruction(leader_id: str, entry: dict, lang: str) -> str:
    """리더별 시스템 프롬프트 생성."""
    name = entry.get("name", "")
    specialty = entry.get("specialty", "")
    profile = entry.get("profile", "")
    personality = _build_personality(leader_id, entry)
...
async def chat_leader(request: Request):
    ...
    async def _stream():
        try:
            ...
            # System instruction
            sys_instruction = _build_system_instruction(leader_id, entry, lang)
