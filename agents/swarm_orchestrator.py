...
class SwarmOrchestrator:
    ...
    def _build_domain_index(self):
        ...
        self.domain_keywords = {
            ...
            "L02": ["부동산", "등기", "소유권", "지분", "매매"],  # 부동산법
            "L03": ["건설", "공사", "하자", "설계"],  # 건설법
            "L04": ["재개발", "재건축", "조합", "분양"],  # 재개발·재건축
            "L05": ["의료", "진료", "수술", "병원", "의사"],  # 의료법
            "L06": ["손해배상", "과실", "배상", "보상"],  # 손해배상
            "L07": ["교통사고", "사고", "과실", "보험"],  # 교통사고
            "L08": ["임대차", "전세", "월세", "보증금", "임차"],  # 임대차
            "L09": ["국가계약", "입찰", "계약"],  # 국가계약
            "L10": ["민사집행", "경매", "압류", "배당", "강제집행"],  # 민사집행
            "L11": ["채권", "추심", "변제", "채무"],  # 채권추심
            "L12": ["등기", "경매", "낙찰", "배당"],  # 등기·경매
            "L13": ["상사", "회사", "주주", "이사", "상법"],  # 상사법
            "L14": ["회사법", "M&A", "인수합병", "주식"],  # 회사법·M&A
            "L15": ["스타트업", "투자", "벤처", "스톡옵션"],  # 스타트업
            "L16": ["보험", "보험금", "피보험자", "약관"],  # 보험
            "L17": ["국제거래", "무역", "중재", "외국"],  # 국제거래
            "L18": ["에너지", "자원", "전력", "광업"],  # 에너지·자원
            "L19": ["해상", "항공", "선박", "운송"],  # 해상·항공
            "L20": ["조세", "금융", "세금", "과세", "은행"],  # 조세·금융
            "L21": ["IT", "보안", "해킹", "데이터", "정보보호"],  # IT·보안
            "L22": ["형사", "고소", "처벌", "범죄", "기소"],  # 형사법
            "L23": ["엔터테인먼트", "연예", "계약"],  # 엔터테인먼트
            "L24": ["조세불복", "심판", "이의신청"],  # 조세불복
            "L25": ["군형법", "군대", "군사"],  # 군형법
            "L26": ["지식재산권", "특허", "상표", "저작권"],  # 지식재산권
            "L27": ["환경", "오염", "배출", "환경법"],  # 환경법
            "L28": ["무역", "관세", "수입", "수출"],  # 무역·관세
            "L29": ["게임", "콘텐츠", "아이템"],  # 게임·콘텐츠
            "L30": ["노동", "해고", "임금", "근로", "퇴직"],  # 노동법
            "L31": ["행정", "행정처분", "취소", "허가"],  # 행정법
            "L32": ["공정거래", "독점", "담합", "불공정"],  # 공정거래
            "L33": ["우주항공", "위성", "발사체"],  # 우주항공
            "L34": ["개인정보", "GDPR", "정보주체"],  # 개인정보
            "L35": ["헌법", "위헌", "기본권", "헌재"],  # 헌법
            "L36": ["문화", "종교", "문화재"],  # 문화·종교
            "L37": ["소년법", "청소년", "미성년"],  # 소년법
            "L38": ["소비자", "피해", "환불", "약관"],  # 소비자
            "L39": ["정보통신", "통신", "망"],  # 정보통신
            "L40": ["인권", "차별", "평등"],  # 인권
            "L41": ["이혼", "가족", "양육", "위자료", "혼인"],  # 이혼·가족
            "L42": ["저작권", "표절", "침해"],  # 저작권
            "L43": ["산업재해", "산재", "업무상"],  # 산업재해
            "L44": ["사회복지", "복지"],  # 사회복지
            "L45": ["교육", "학교", "청소년"],  # 교육·청소년
            "L46": ["보험", "연금", "국민연금"],  # 보험·연금
            "L47": ["벤처", "신산업", "규제샌드박스"],  # 벤처·신산업
            "L48": ["문화예술", "예술", "미술"],  # 문화예술
            "L49": ["식품", "보건", "위생"],  # 식품·보건
            "L50": ["다문화", "이주", "외국인"],  # 다문화·이주
            "L51": ["종교", "전통", "종교법인"],  # 종교·전통
            "L52": ["광고", "언론", "명예훼손", "언론중재"],  # 광고·언론
            "L53": ["농림", "축산", "농지"],  # 농림·축산
            "L54": ["해양", "수산", "어업"],  # 해양·수산
            "L55": ["과학기술", "R&D", "연구"],  # 과학기술
            "L56": ["장애인", "복지", "편의시설"],  # 장애인·복지
            "L57": ["상속", "신탁", "유언", "유산", "명의신탁"],  # 상속·신탁
            "L58": ["스포츠", "레저", "체육"],  # 스포츠·레저
            "L59": ["데이터", "AI윤리", "알고리즘"],  # 데이터·AI윤리
            "L60": ["시스템", "통합", "총괄"],  # 시스템 총괄
        }
    ...
    def select_leaders(self, query: str, detected_domains: List[Tuple[str, int]] = None) -> List[Dict]:
        ...
        logger.info(f"✅ {len(selected_leaders)}명 리더 선택: {', '.join([f'{l['name']}({l.get('specialty', '?')})' for l in selected_leaders])}")
