...
| 항목 | 상세 |
|:---|:---|
| **분석 엔진** | Google Gemini 2.5 Flash (`gemini-2.5-flash`) |
| **검증 소스** | 법제처 DRF JSON API (`law.go.kr`) 실시간 10종 SSOT |
| **리더 구성** | 60개 도메인 특화 라우팅 + C-Level 3명 (총 63 페르소나) |
| **응답 구조** | 결론 → 이유 → 행동계획 → 법률근거 → 추가정보 |
| **안전장치** | SafetyGuard + CircuitBreaker + 헌법 준수 검증 + DRF 전수검증 |
| **인프라** | GCP Cloud Run (asia-northeast3) + Firebase Hosting |
| **버전** | v60.0.0 (2026-02-28 기준) |
| **법률 LLM** | LawmadiLM v4.0 (Qwen3-1.7B QLoRA, 85,909 Q&A 학습) — ⚠️ 현재 비활성화 |
| **모델 체인** | Flash → Pro → Lite (자동 폴백) |
| **협의 시스템** | CSO 서연 주재 리더 회의 + 리더 간 인수인계 |
...
사용자 질문
  ...
  │ → Gemini Flash 분석 (티어/리더/분야 배정)                    │
  │ → 3-Tier 라우팅: 인사→시스템소개 / 비법률→안내 / 법률→파이프라인  │
  └────────────────────────────────────────────────────────────┘
        ↓
  ┌─ Stage 2: LawmadiLM 초안 생성 (⚠️ 현재 비활성화) ────────────┐
  │ LawmadiLM API 호출 (법률명, 조문, 판례, 결론 초안)             │
  │ 최대 150 토큰 / 타임아웃 15초 / 실패 시 Gemini Fallback       │
  │ ※ 서킷브레이커 내장 (5회 연속실패→60초 open)                   │
  │ ※ 현재 ENABLE_LAWMADILM=false, Cloud Run 서비스 삭제됨       │
  └────────────────────────────────────────────────────────────┘
        ↓
  ┌─ Stage 3: Gemini Flash 작성+보강 (핵심) ─────────────────────┐
  │ 시스템 인스트럭션 + 리더 페르소나 + SSOT 캐시 컨텍스트          │
  │ DRF 9종 도구 자동 호출 (automatic_function_calling)           │
  │ SSOT 캐시 99% 최우선 참조 원칙 적용                           │
  │ 일반 3,500 / 전문가 5,000 토큰 출력                          │
  └────────────────────────────────────────────────────────────┘
        ↓
  ┌─ Stage 4: DRF 전수검증 + 헌법 준수 체크 ────────────────────┐
  │ 응답 내 법령 참조 + 판례번호 + 헌재결정례 추출                  │
  │ DRF 실시간 교차 검증 (법조문·판례·헌재결정 번호-내용 일치)       │
  │ 헌법적 원칙 위배 시 경고/교정 삽입                              │
  │ 30% 이상 미검증 시 Fail-Closed                               │
  │ 감사 로그 기록 (Cloud SQL)                                    │
  └────────────────────────────────────────────────────────────┘
        ↓
  최종 응답 반환
...
질문이 들어오면 CSO 서연이 주재하는 리더 회의를 통해 최적의 전문가를 배정합니다.

### 협의 유형

| 유형 | 조건 | 내용 | 추가 시간 |
|:---|:---|:---|:---:|
| **전체 협의** | 첫 질문 (`is_first_question=true`) | CSO 서연 + 2명 리더 3턴 회의 | +2~4초 |
| **인수인계** | 리더 변경 시 | 전임↔후임 리더 2턴 대화 | +1~2초 |
| **없음** | 같은 분야 후속 질문 | 기존 리더 유지, 회의 스킵 | 0초 |
...
사용자 질문
  ↓
should_deliberate(query, current_leader, is_first_question)
  ├─ "full"    → generate_deliberation() → CSO + 2리더 3턴 회의 (Turn2+3 병렬)
  ├─ "handoff" → generate_handoff()      → 전임↔후임 2턴 대화 (병렬)
  └─ "none"    → 스킵
  ↓ (asyncio.gather로 파이프라인과 병렬 실행)
최종 응답 + 협의 UI
```

### 기술 상세

- **실제 리더 페르소나 주입**: `_build_cso_persona()`, `_single_leader_call()`
- **Gemini 호출**: 턴별 개별 호출, `max_output_tokens=300`, `thinking_budget=0`
- **타임아웃**: per-turn `asyncio.wait_for` 5초, 외부 8초
- **실패 시**: graceful fallback (회의 스킵, 기존 라우팅 그대로)

### 모델 폴백 체인

응답 생성 시 Gemini 모델이 Quota 초과 등으로 실패하면 자동으로 다음 모델로 전환합니다.

```
gemini-2.5-flash (기본) → gemini-2.5-pro → gemini-2.0-flash-lite
         ↓ Quota 초과             ↓ Quota 초과           ↓ 최종
    자동 전환 (on_quota_error)   자동 전환              에러 반환
```

관리 모듈: `core/model_fallback.py` (`MODEL_CHAIN`, `get_model()`, `on_quota_error()`)

---

## 🤖 LawmadiLM (법률 특화 LLM)

> ⚠️ **현재 비활성화**: LawmadiLM은 n_ctx=4096 초과 500에러 및 타임아웃 반복으로 비활성화되었습니다 (`ENABLE_LAWMADILM=false`). Cloud Run 서비스(lawmadilm-llm, lawmadilm-rag, lawmadilm-api)도 삭제되었습니다. Stage 2는 스킵되며 Gemini Flash가 직접 응답합니다.
...
| 서비스 | CPU | 메모리 | 인스턴스 | 상태 |
|:---|:---:|:---:|:---:|:---:|
| `lawmadilm-llm` (LLM 추론) | 4 vCPU | 16 GiB | 1~2 | ❌ 삭제 (2026-02-28) |
| `lawmadilm-rag` (RAG 검색) | 2 vCPU | 8 GiB | 0~3 | ❌ 삭제 (2026-02-28) |
| `lawmadilm-api` (API 게이트웨이) | 1 vCPU | 512 MiB | 0~1 | ❌ 삭제 (2026-02-28) |
...
| `GET` | `/api/admin/system-metrics` | 시스템 메트릭 (p50/p95/p99, 캐시율) |
| `GET` | `/api/admin/frontend-errors` | 프론트엔드 JS 에러 로그 |
| `GET` | `/api/admin/frontend-perf` | 프론트엔드 Core Web Vitals |
| `POST` | `/api/errors` | 프론트엔드 에러 수집 |
| `POST` | `/api/perf` | 프론트엔드 성능 수집 |
...
```json
{
  ...
  "latency_ms": 3240,
  "deliberation": [
    {"speaker": "CSO 서연", "message": "임대차 분쟁 사안입니다..."},
    {"speaker": "온유", "message": "주택임대차보호법 관점에서..."},
    {"speaker": "CSO 서연", "message": "온유 리더에게 배정하겠습니다."}
  ],
  "handoff": null,
  "current_leader": "L08"
}
...
| LLM | Google Gemini | 2.5 Flash | 법률 분석 AI (google-genai SDK) |
| 모델 체인 | Flash → Pro → Lite | 자동 폴백 | Quota 초과 시 자동 전환 (`core/model_fallback.py`) |
| LLM (초안) | LawmadiLM | v4.0 (Qwen3-1.7B) | 법률 초안 생성 (Stage 2) — ⚠️ 현재 비활성화 |
...
| lawmadi-os-v60 | Cloud Run | 2 vCPU / 4 GiB, min 0 ~ max 10 인스턴스 |
| ~~lawmadilm-llm~~ | ~~Cloud Run~~ | ❌ 삭제됨 (2026-02-28) |
| ~~lawmadilm-rag~~ | ~~Cloud Run~~ | ❌ 삭제됨 (2026-02-28) |
| 타임아웃 | 300초 | 동시성: 15 |
...
├── law_cache.json                  # 키워드 매칭 캐시 (548 법률, 14,088 키워드)
│
├── core/
...
│   ├── deliberation.py             # 리더 협의 & 인수인계 시스템
│   │                               # should_deliberate(), generate_deliberation(), generate_handoff()
│   ├── model_fallback.py           # 모델 폴백 체인 (Flash→Pro→Lite)
│   ├── classifier.py               # NLU 분류기 (_NLU_INTENT_PATTERNS, _NLU_PRIORITY)
│   ├── pipeline.py                 # 4단계 파이프라인 (_LEADER_LAW_BOOST, 서킷브레이커)
│   ├── constants.py                # 시스템 상수 (DEFAULT_GEMINI_MODEL 등)
│   ├── auth.py                     # 공통 인증 (Bearer 파싱, hmac.compare_digest)
│   ├── metrics.py                  # 중앙 메트릭 수집기 (p50/p95/p99, 캐시율)
...
│   ├── db_client_v2.py             # 고수준 DB ops (audit, visitor, chat, verification, frontend logs)
│   ├── db_driver_adapter.py        # pg8000/psycopg2 드라이버 어댑터
│   └── validator.py                # SHA-256 무결성 서명
│
├── routes/
│   └── legal.py                    # /ask, /ask-stream, /ask-expert 엔드포인트
│                                   # 협의+파이프라인 병렬화, SSE 스트리밍
│
├── engines/
│   ├── temporal_v2.py              # 시계열 분석 엔진
│   │                               # 행위시법 vs 재판시법, 부칙/경과조치 처리
│   ├── response_verifier.py        # DRF SSOT 검증 (다단계 파싱 복구)
...
│       ├── purify.min.js           # DOMPurify 3.2.4 (XSS 방지)
│       ├── lawmadi-monitor.js      # 프론트엔드 모니터링 (JS에러 + Core Web Vitals)
...
│
├── tests/
│   ├── test_leader_matching.py     # NLU 라우팅 테스트 (264건)
│   └── test_verifier_parse.py      # Verifier 파싱 테스트 (18건)
│                                   # 총 282건, 전체 통과
│
├── scripts/
│   └── deploy_cloud_run_v60.sh     # Cloud Run 수동 배포 스크립트
│
├── .github/
│   └── workflows/
...
Env:           ENABLE_LAWMADILM=false, LAWMADILM_TIMEOUT=3
Secrets:       GEMINI_API_KEY, GEMINI_KEY, LAWGO_DRF_OC, ANTHROPIC_API_KEY,
               DATABASE_URL, CLOUD_SQL_INSTANCE, DB_USER, DB_PASS, DB_NAME,
               MCP_API_KEY, INTERNAL_API_KEY, PREMIUM_KEYS
```
...
Cloud SQL (PostgreSQL 17 Enterprise, `db-f1-micro`, HDD 10GB, asia-northeast3) 12+ 테이블:
...
-- 기타 테이블: audit_logs, rate_limit_tracker, response_verification,
-- frontend_errors, frontend_perf, lawyer_inquiries, feedback 등
-- 앱 시작 시 자동 생성 (init_tables() 체인)
```
...
*Copyright (c) 2026 Jainam Choe (최재남). All rights reserved.*
*최종 업데이트: 2026-02-28 | 운영 환경: GCP asia-northeast3*
