...
| 항목 | 상세 |
|:---|:---|
| **분석 엔진** | Google Gemini 3 Flash (`gemini-3-flash-preview`) + 자동 폴백 체인 |
| **모델 폴백** | gemini-3-flash-preview → gemini-2.5-flash → gemini-2.5-flash-lite (429/404 자동 전환) |
| **검증 소스** | 법제처 DRF JSON API (`law.go.kr`) 실시간 10종 SSOT |
| **리더 구성** | 60개 도메인 특화 라우팅 + C-Level 3명 (총 63 페르소나) |
| **파이프라인** | 3-Stage Hybrid: 분류+RAG 병렬 → LawmadiLM 강화 답변 → DRF 전수 검증 |
...
| **버전** | v60.0.0 (2026-03-07 기준) |
| **법률 LLM** | LawmadiLM v4.0 (Qwen3-1.7B QLoRA, 85,909 Q&A 학습) — 주력 답변 생성기 |
...
| `POST` | `/api/paddle/send-otp` | Email OTP 발송 | — |
| `POST` | `/api/paddle/verify-otp` | Email OTP 검증 + 세션 발급 | — |
...
| `GET` | `/conversion` | 전환율 분석 |
| `GET` | `/retention` | 리텐션 분석 |
| `GET` | `/cost-estimate` | 비용 추정 |
...
| 19 | **헤더 배지** | Analysis \| Verify 아이콘+텍스트 (브랜드명 제거) |
| 20 | **모바일 헤더 정렬** | 로그인+언어전환 좌측, 즐겨찾기+다크모드 우측 균형 배치 |
...
| LLM (분류/폴백) | Google Gemini | 3 Flash Preview | 질문 분류 + 폴백 답변 (google-genai SDK) |
| LLM 폴백 체인 | Gemini 자동 전환 | 3단계 | 3-Flash → 2.5-Flash → 2.5-Flash-Lite (429/404 자동 전환) |
| LLM (주력 답변) | LawmadiLM | v4.0 (Qwen3-1.7B) | 법률 강화 답변 생성 (Stage 2) |
...
| lawmadi-os-v60 | Cloud Run | 2 vCPU / 2 GiB, 최소 1 최대 10 인스턴스 |
...
│   ├── pipeline.py                 # 3-Stage Hybrid 법률 파이프라인 (분류+RAG→LawmadiLM→DRF검증)
│   ├── classifier.py               # NLU 의도 감지 + 키워드 + Gemini 분류 + 리더 선택
│   ├── model_fallback.py           # Gemini 모델 자동 전환 체인 (3-Flash→2.5-Flash→Lite)
...
│   ├── paddle.py                   # /api/paddle/* — Paddle Billing + Email OTP + 크레딧
│   ├── admin.py                    # /dashboard, /conversion, /retention
...
Secrets:       GEMINI_API_KEY, GEMINI_KEY, LAWGO_DRF_OC, ANTHROPIC_API_KEY, CLOUD_SQL_INSTANCE, DB_USER, DB_PASS, DB_NAME, MCP_API_KEY, API_KEYS, JWT_SECRET, PADDLE_WEBHOOK_SECRET, PADDLE_API_KEY, PADDLE_CLIENT_TOKEN
```
...
Cloud SQL (PostgreSQL 18) 4개 주요 테이블:
...
CREATE TABLE drf_cache (
    ...
    signature       VARCHAR(64)  NOT NULL,    -- SHA-256 무결성 서명
...
-- 인덱스
CREATE INDEX idx_drf_cache_expires ON drf_cache (expires_at);
...
*Copyright (c) 2026 Jainam Choe (최재남). All rights reserved.*
*최종 업데이트: 2026-03-07 | 운영 환경: GCP asia-northeast3 | Gemini 3 Flash + LawmadiLM v4.0*
