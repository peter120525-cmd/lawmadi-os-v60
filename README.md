...
사용자 질문
  ...
  │ 최대 60 토큰 / 타임아웃 5초 / 실패 시 Gemini Fallback         │
  └────────────────────────────────────────────────────────────┘
        ↓
...
| 18 | **Rate Limit 안내** | Free 100회/4시간, Premium 200회/4시간 슬라이딩 윈도우 |
...
| 페이지 | index / leaders / clevel / about / leader-profile / privacy / terms | 7개 주요 페이지 (각 KO/EN) |
...
│
├── core/                           # 핵심 비즈니스 로직
│   ├── constants.py                # 전역 상수 (OS_VERSION, GEMINI_MODEL)
│   ├── pipeline.py                 # 4-Stage 법률 분석 파이프라인
│   ├── classifier.py               # Gemini 기반 질문 분류 + 리더 선택
...
│   └── static.py                   # /, /en, /leaders, /about, /terms, /privacy
│
...
│       ├── leader-profile.html     # 개별 리더 프로필 페이지
│       ├── leader-profiles.json    # 리더 프로필 상세 데이터 (118KB)
...
Secrets:       GEMINI_API_KEY, GEMINI_KEY, LAWGO_DRF_OC, ANTHROPIC_API_KEY, CLOUD_SQL_INSTANCE, DB_USER, DB_PASS, DB_NAME, MCP_API_KEY, API_KEYS
```
...
*Copyright (c) 2026 Jainam Choe (최재남). All rights reserved.*
*최종 업데이트: 2026-03-07 | 운영 환경: GCP asia-northeast3*
