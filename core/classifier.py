...
_NLU_INTENT_PATTERNS: Dict[str, List[str]] = {
    ...
    "L09": [
        r"(?:입찰|낙찰|유찰).*(?:자격|조건|취소|이의|부정|담합|어떻게|해야)",
        r"(?:조달|조달청|나라장터).*(?:등록|입찰|계약|절차|어떻게|해야)",
        r"(?:국가\s*계약|관급\s*공사|정부\s*조달).*(?:분쟁|이의|해제|어떻게|해야|절차)",
    ...
    "L17": [
        r"(?:국제\s*계약|국제계약|해외\s*거래).*(?:분쟁|준거법|중재|소송|어떻게|해야)",
    ...
    "L28": [
        r"(?:수입\s*통관|수입통관|통관\s*절차).*(?:지연|거부|문제|어떻게|해야|방법)",
        r"(?:관세|관세율|관세\s*환급).*(?:부과|이의|환급|어떻게|해야|절차|감면)",
        r"(?:FTA|원산지\s*증명|원산지증명).*(?:발급|적용|혜택|어떻게|해야|절차)",
        r"(?:수입|수출).*(?:규제|금지|허가|신고|통관).*(?:어떻게|해야|절차|방법)",
    ],
    ...
    "L32": [
        ...
        r"(?:카르텔|담합|입찰\s*담합).*(?:신고|제재|과징금|어떻게|해야|의심)",
    ...
    "L39": [
        r"(?:통신\s*분쟁|통신분쟁|통신\s*요금).*(?:이의|조정|과다|분쟁|어떻게|해야)",
        r"(?:방송\s*규제|방송규제|방송\s*심의).*(?:위반|제재|이의|어떻게|해야)",
    ...
    "L46": [
        r"(?:국민\s*연금|국민연금).*(?:수령|신청|미납|체납|반환|감액|어떻게|해야|문제)",
        r"(?:건강\s*보험|건강보험|건보).*(?:피부양자|자격|보험료|체납|이의|어떻게|해야)",
        r"(?:퇴직\s*연금|퇴직연금|IRP|DB|DC).*(?:수령|해지|중도\s*인출|어떻게|해야|문제)",
        r"(?:연금|기금).*(?:수급|자격|감액|지급|정지|어떻게|해야|신청)",
    ],
